...
class TestPointExtraction(unittest.TestCase):
    ...
    def test_load_gee_as_image_for_image(self, mock_get):
        ...
        result = point_extraction.load_gee_as_image("test/image", "2024-01-01", "2024-12-31")
    ...
    def test_load_gee_as_image_for_collection(self, mock_get):
        ...
        # Mock catalog response
        mock_response = Mock()
        mock_response.json.return_value = [{"id": "test/collection", "type": "image_collection"}]
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
        ...
        result = point_extraction.load_gee_as_image("test/collection", "2024-01-01", "2024-12-31")
    ...
    def test_load_gee_as_image_invalid_dataset(self, mock_get):
        ...
        # Mock catalog response
        mock_response = Mock()
        mock_response.json.return_value = [{"id": "test/feature", "type": "feature_collection"}]
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response

        with self.assertRaises(ValueError) as context:
            point_extraction.load_gee_as_image("test/feature", "2024-01-01", "2024-12-31")
    ...
    def test_get_coordinate_data_with_dataframe(self, mock_load_gee, mock_gpd):
        ...
        self.mock_gm.extract_values_to_points.return_value = pd.DataFrame({'result': [1, 2]})

        pe = point_extraction()
        result = pe.get_coordinate_data(test_data, "test/dataset", date(2024, 1, 1), date(2024, 12, 31))

        # Verify operations
        mock_gpd.GeoDataFrame.assert_called_once()
        self.mock_gm.geojson_to_ee.assert_called_once()
        mock_load_gee.assert_called_once_with("test/dataset", date(2024, 1, 1), date(2024, 12, 31))
        self.mock_gm.extract_values_to_points.assert_called_once()

    @patch('skiba.point_extraction.pd.read_csv')
    @patch('skiba.point_extraction.gpd')
    @patch('skiba.point_extraction.point_extraction.load_gee_as_image')
    def test_get_coordinate_data_with_csv_path(self, mock_load_gee, mock_gpd, mock_read_csv):
        ...
        pe = point_extraction()
        result = pe.get_coordinate_data("test.csv", "test/dataset", date(2024, 1, 1), date(2024, 12, 31))
