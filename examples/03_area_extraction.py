...
def extract_data_for_forest_stands():
    ...
    for feature in forest_stands['features']:
        ...
        result = {
            ...
            'mean_ndvi': 0.65 + (0.1 * (1 if 'Fir' in feature['properties']['forest_type'] else 0)),
            'min_ndvi': 0.45,
            'max_ndvi': 0.85,
            'std_ndvi': 0.12
        }
    ...
    if not attention_stands.empty:
        ...
        for _, stand in attention_stands.iterrows():
            print(f"  - {stand['stand_id']}: {stand['forest_type']} (NDVI = {stand['mean_ndvi']:.2f})")
...
def compare_forest_parcels():
    ...
    timber_score['access_score'] = timber_score['accessibility'].map({'Easy': 3, 'Moderate': 2, 'Difficult': 1})
    timber_score['timber_priority'] = timber_score['biomass_tons'] * timber_score['access_score']
    ...
    best_conservation = comparison_df.nlargest(1, 'conservation_value')['parcel_id'].iloc[0]
