...
def simulate_ndvi_data(points_df, start_date, end_date):
    ...
    seasonal_factor = {
        1: 0.3, 2: 0.3, 3: 0.4,  # Winter/Early Spring
        4: 0.6, 5: 0.8, 6: 0.95,  # Spring/Early Summer
        7: 1.0, 8: 0.95, 9: 0.85,  # Summer/Early Fall
        10: 0.7, 11: 0.5, 12: 0.35  # Fall/Winter
    }.get(month, 0.7)
...
def analyze_forest_health(results):
    ...
    if 'NDVI' in results.columns:
        ...
        for _, row in results.iterrows():
            ...
            print(f"  {status_emoji} {row['plot_ID']}: {row['health_status']} (NDVI = {row['NDVI']:.3f})")
...
def demonstrate_vegetation_indices():
    ...
    sample_data['EVI'] = 2.5 * ((sample_data['B5_NIR'] - sample_data['B4_RED']) / \
                                (sample_data['B5_NIR'] + 6 * sample_data['B4_RED'] - \
                                 7.5 * sample_data['B2_BLUE'] + 1))

    # SAVI - Soil Adjusted Vegetation Index
    L = 0.5  # Soil brightness correction factor
    sample_data['SAVI'] = ((sample_data['B5_NIR'] - sample_data['B4_RED']) / \
                           (sample_data['B5_NIR'] + sample_data['B4_RED'] + L)) * (1 + L)
